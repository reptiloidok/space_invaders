...
# изображения
bg_img = pg.image.load('space_invaders/src/background.png')
icon_img = pg.image.load('space_invaders/src/ufo.png')
...
font = pg.font.Font('space_invaders/src/04B_19.TTF', 48)
...
# игрок
player_img = pg.image.load('space_invaders/src/player.png')
...
# пуля
bullet_img = pg.image.load('space_invaders/src/bullet.png')
...
# противник
enemy_img = pg.image.load('space_invaders/src/enemy.png')
angry_enemy_img = pg.image.load('space_invaders/src/enemy2.png')
...
def bullet_create():
    global bullet_y, bullet_x, bullet_alive, bullet_width
    bullet_alive = True
...
def enemy_model():
    ...
    global enemy_y, enemy_x, bullet_alive, score, flag_wall
    if enemy_x + enemy_width > screen_width or enemy_x <= 0:
        flag_wall = not flag_wall
    enemy_y += enemy_dy
    if score >= 200:
        if flag_wall:
            enemy_x += enemy_dx
            ...
            enemy_x -= enemy_dx
    if enemy_y + enemy_height > screen_height:
        game_over()
        enemy_create()
...
def display_redraw():
    ...
    if score < 200:
        ...
        display.blit(angry_enemy_img, (enemy_x, enemy_y))
    score_img = sys_font.render('SCORE  ' + str(score), True, 'white')
...
def game_over():
    display_redraw()
    global score
    over_img = sys_font.render('GAME OVER ', True, 'white')
    display.blit(over_img, ((screen_width - over_img.get_size()[0]) / 2, 250))
    score_img = sys_font.render('SCORE  ' + str(score), True, 'white')
    display.blit(score_img, ((screen_width - score_img.get_size()[0]) / 2, 300))
    text_img = sys_font.render('press any button to continue', True, 'white')
    display.blit(text_img, ((screen_width - text_img.get_size()[0]) / 2, 350))
    pg.display.update()
    while True:
